...
| Windows: the camera preview says "unavailable", or the terminal says `CAMERA NOT WORKING` / `can't grab frame` | Windows lets only one program use the webcam at a time. Close every other browser tab showing the tutor page, and apps like Zoom, Teams or Camera. Then run `git pull`, restart the tutor, and press **Ctrl+F5** on the page so it loads the latest version. Laptops with face login (Windows Hello) can have an infrared camera first: the app tries camera 1 automatically, or set `CAMERA_INDEX = 1` in `config.py`. |
| The voice starts late, or focus tracking feels slow | Plug the laptop in (battery saver slows the CPU a lot), close other heavy apps and browser tabs, and use headphones. Still slow? Quit the tutor and run the speed check: `.venv\Scripts\python scripts\diagnose.py` (Mac/Linux: `.venv/bin/python scripts/diagnose.py`). It takes about a minute and writes `logs/diagnose.txt`; share that file together with `logs/tutor.log` from a session where you talked a few times. |
